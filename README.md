...
| terraform/                   | Google Cloud infrastructure-as-code for GCS and BigQuery                |
| kestra/flows/                | Kestra workflow definitions                                             |
| scripts/                     | Helper scripts for provisioning and cloud/Kestra execution              |
...
| keys/                        | Optional ignored local folder for Google service account JSON files      |
| README.md                    | Main project entry point                                                |
| docker-compose.yaml          | Reproducible local stack                                                |
| pyproject.toml               | Python project metadata and dependencies                                |
| .env.example                 | Example local and Google Cloud environment configuration                 |
...
#### Terraform and final cloud pipeline
- [Terraform](https://developer.hashicorp.com/terraform/install) installed
- A Google Cloud project ID provided by the reviewer
- Billing enabled for the Google Cloud project
- Google Cloud Storage and BigQuery APIs enabled for the project
- A Google service account JSON key file with permissions to create and use the GCS bucket and BigQuery dataset

### 1.3 Environment Variables

The project uses a local `.env` file for reproducible local development, Docker Compose execution, Terraform provisioning, and cloud pipeline execution.
...
| `GOOGLE_APPLICATION_CREDENTIALS` | Path to the Google service account JSON file used by Terraform and local cloud commands |
| `GOOGLE_CLOUD_PROJECT`       | Reviewer-provided Google Cloud project ID                               |
| `OBJECT_STORAGE_BUCKET`      | GCS data lake bucket name, generated by the provisioning script if empty |
| `BIGQUERY_DATASET`           | BigQuery warehouse dataset name, generated by the provisioning script if empty |
...
> [!NOTE]
> The local values in `.env.example` are demo defaults for reproducible testing only. The Google Cloud values are placeholders: `scripts/provision_gcp_infra.sh` asks for the project ID and service account path if needed, then generates unique bucket and dataset names.
